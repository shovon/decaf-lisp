(function (module) {
   ...
   */
  var tokenize = module.exports.tokenize = function (src) {
    return src.match(/[\(\)]|[^\s\(\)]+/g);
  };

  /**
   * In JavaScript, `typeof []` returns `"object"` 
   * (i.e. `typeof [] == typeof {}`). This function checks to see whether or not
   * the passed-in object is an array.
   *
   * @param an object
   ...
   * @param arr the array to check the value against.
   ...
   */
  var buildTree = module.exports.buildTree = function (tokens) {
    var i
      , len
      , retval
  ...
  var outputStatement = module.exports.outputStatement = function (tree) {
    var i
      ...
      , str = (function () {
        if (tree.callee == null) {
          throw new Error(JSON.stringify(tree, null, 2));
        }
  ...
  var outputFunction = module.exports.outputFunction = function (tree) {
    var p = [];
    p.push(
      FUNCTION_DEPOT_NAME, '["', tree.name, '"] = function ('
        , tree.params.join(', ')
      , ') {'
      ,   'return '
    );
    if (tree.code.type === 'lambda') {
      p.push(outputLambda(tree.code));
    } else {
      p.push(outputStatement(tree.code));
    }
    p.push('}');
    return p.join('');
  };

  var DEFAULT_FUNCTIONS = module.exports.DEFAULT_FUNCTIONS = (function () {
    var p = []
      , create = function (name, f) {
        var func = [];
        func.push(FUNCTION_DEPOT_NAME, '["', name, '"] = ', f.toString());
        return p.push(func.join(''));
      };
    create('+', function (x, y) {
      return x + y;
    });
  ...
  var build = module.exports.build = function (ast) {
    var i, len, p = [];

    p.push('var ' + FUNCTION_DEPOT_NAME + ' = {}');
    p.push(DEFAULT_FUNCTIONS);

    for (i = 0, len = ast.length; i < len; i++) {
      if (ast[i].type === "call") {
        p.push(outputStatement(ast[i]));
      } else if(ast[i].type === "function") {
        p.push(outputFunction(ast[i]));
      }
    }
  ...
  if (describe) {
    describe(
      ...
      , function (mod, exp, req) {
        exp = module.exports;
      }
    );
  }

  this.decafLisp = module.exports;
